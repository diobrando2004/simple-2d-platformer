	...
	private void drawSkele(Graphics g, int xlvlOffset) {
		...
		if (sk.isActive()){
			g.drawImage(SkeleArr[sk.getEnemyState()][sk.getAniIndex()], 
					(int)sk.getHitbox().x - xlvlOffset+80+sk.flipX(), 
					(int)sk.getHitbox().y-52, 64*2*(sk.flipW()), 64*2, null);
			g.setColor(Color.red);
			g.drawRect((int)sk.getHitbox().x-xlvlOffset,(int) sk.getHitbox().y,(int) sk.getHitbox().getWidth(),(int)sk.getHitbox().getHeight() );
