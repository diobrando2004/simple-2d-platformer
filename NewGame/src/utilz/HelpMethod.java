...
public class HelpMethod {
	...
	public static boolean isTileSolid(int xTile, int yTile,int[][] lvlData) {
		int value;
		if (xTile == 0||yTile == 14 ||yTile == 0||yTile>=14) value = 1;
		else value = lvlData[yTile][(int) xTile];
	...
	public static boolean isFloor(Rectangle2D.Float hitbox,float xSpeed, int[][] lvlData, int walkDir) {
		if (walkDir == 0) return IsSolid(hitbox.x-xSpeed, hitbox.y+hitbox.height+1, lvlData);
		else return IsSolid(hitbox.x+xSpeed+hitbox.width, hitbox.y+hitbox.height+1, lvlData);
	}
